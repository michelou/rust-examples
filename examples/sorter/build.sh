...
compile() {
    ...
    [[ -d "$TARGET_DEPS_DIR" ]] || mkdir -p "$TARGET_DEPS_DIR"
...
compile_rust() {
    ...
    local source_files=
    local n=0
    for f in $($FIND_CMD "$SOURCE_DIR/" -type f -name "*.rs" 2>/dev/null); do
    ...
    local rust_deps_opts="-L \"dependency=$TARGET_DEPS_DIR\" --extern \"rand=$TARGET_DEPS_DIR/librand.rlib\" --extern \"rayon=$TARGET_DEPS_DIR/librayon.rlib\""
    local rustc_opts="$rust_crate_opts --edition $EDITION --out-dir \"$TARGET_DIR\" --target \"$TARGET_TRIPLE\""

    if [[ $DEBUG -eq 1 ]]; then
        debug "\"$RUSTC_CMD\" $rustc_opts $source_files $rust_deps_opts"
    elif [[ $VERBOSE -eq 1 ]]; then
        echo "Compile $n_files to directory \"${TARGET_DIR/$ROOT_DIR\//}\"" 1>&2
    fi
    eval "\"$RUSTC_CMD\" $rustc_opts $source_files $rust_deps_opts"
    if [[ $? -ne 0 ]]; then
...
compile_deps() {
    # TODO download and unzip libraries if not yet present (see batch file)
    for f in $($FIND_CMD "$LIB_DIR/" -type f -name "*.zip" 2>/dev/null); do
        local dir_name="$LIB_DIR/$(basename $f .zip)"
        for g in $($FIND_CMD "$dir_name/" -type f -name "lib*.rlib" 2>/dev/null); do
            cp $g $TARGET_DEPS_DIR/
        done
    done
}
