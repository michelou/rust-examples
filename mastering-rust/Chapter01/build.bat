...
@rem                    _ROOT_DIR, _SOURCE_DIR, _TARGET_DIR
:env
set _BASENAME=%~n0
set "_ROOT_DIR=%~dp0"
...
set _PROJECT_URL=github.com/%USERNAME%/rust-examples
set _PROJECT_VERSION=1.0-SNAPSHOT

set _DEFAULT_RUN=arrays
...
if exist "%__PROPS_FILE%" (
    ...
    if defined _default_run set _DEFAULT_RUN=!_default_run!
)
goto :eof
...
@rem output parameter: _MAIN
:set_main
set _MAIN=

set "__ARG=%~1"
if not exist "%_SOURCE_DIR%\%__ARG%.rs" (
    echo %_ERROR_LABEL% Rust source file "%__ARG%.rs" not found 1>&2
    set _EXITCODE=1
    goto :eof
)
set _MAIN=%__ARG%
